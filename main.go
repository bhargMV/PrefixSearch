/*
    A trie tree, where each node represent a letter, provides three methods.
    - addWord(string)
    - addWords("<comma separated words>")
    - getWordsMatchingPrefix(string) string
    
    Notes:
    - Path from root of the tree to a trie node T represents prefix 
      for every child of T.
    - To minimize query time, every trieNode T deeper than level 3 stores
      all the words whose prefix matches path from root to the trieNode T.
    ...
    Example tree:
      ...
      n    p
           ...
           p {isWord: true}
           |
level 4 -> l {wordsWithPrefix: apple, apples}
           |
           e {isWord: true; wordsWithPrefix: apple, apples}
           |
           s {isWord: true; wordsWithPrefix: apples}
           

 Time complexity:
  Addition - O(length(word))
  Search - O(1)
  
Space Complexity:
   Assume there are N words and word with max length M (M much greater than 3). 
   Assume there are k words for every word length ranging from 1 to M.
   
   At each level, the tree contains N - (level-1)(N/k) where level ranges from 1 to M.
   
   space complexity: Sigma(N - (level-1)(N/k)) [1<=level<=M] = O(NM)
   
   Entire english dictionary is 2 MB and average word size is 5. 
   This program require just 10 MB memory for O(1) prefix query search of words.
*/
...
type trieNode struct {
    ...
    // If this is set to true, path from root to current node is a word. 
    isWord bool
    
    // List of words with the prefix. This will be set for trieNodes deeper
    // than level 3.
    wordsWithPrefix []string
}

func (tr *trieNode) addWords(words string) {
    for _, word := range strings.Split(words, ",") {
        tr.addWord(word)
    }
}

// Function to check if a word is already added to the trie tree.
func (tr *trieNode) isAlreadyAdded(word string) bool {
    for i := 0; i < len(word); i++ {
        letter := string(word[i])
        tn, present := tr.childNodes[letter]
        if !present {
            return false
        }
        tr = tn
...
// Adds a word to the trie tree if not already added.
func (tr *trieNode) addWord(word string) {
    ...
    for i := 0; i < len(word); i++ {
        ...
        if i >= 3 {
            // Add this word to the wordsWithPrefix list of every prefix
            // of size greater than 3.
            tn.wordsWithPrefix = append(tn.wordsWithPrefix, word)
        }
        
        tr.childNodes[letter] = tn
        tr = tn
    }
    
    if len(word) <= 3 {
        // If word has length less than or equal to 3, add this word to the 
        // the wordsWithPrefix list. This is used for search.
        tr.wordsWithPrefix = append(tr.wordsWithPrefix, word)
    }
    tr.isWord = true
}

func (tr *trieNode) getWordsMatchingPrefix(prefix string) []string {
    prefix = strings.TrimSpace(prefix)
    words := make([]string, 0)
    
    // First find the trieNode with the prefix. If the node does not exist,
    // return empty list.
    for i := 0; i < len(prefix); i++ {
        tn, present := tr.childNodes[string(prefix[i])]
        if !present {
            return words
        }
        tr = tn
    ...
    // If prefix has length greater than 3, then just return the 
    // wordsWithPrefix list of the prefix trie node.
    if len(prefix) > 3 {
        return tr.wordsWithPrefix
    }
    
    // If the prefix has length less than or equal to 3, get all the
    // trie nodes at the depth 4 that are under the trie node with this prefix.
    curLevel := make([]*trieNode, 0)
    nextLevel := make([]*trieNode, 0)
    curLevel = append(curLevel, tr)
    for i := len(prefix); i <= 3; i++ {
        for _, tn := range curLevel {
            if tn.isWord {
                words = append(words, tn.wordsWithPrefix...)
            }
            
            for _, cn := range tn.childNodes {
               nextLevel = append(nextLevel, cn) 
            }
        }
        curLevel = nextLevel
        nextLevel = make([]*trieNode, 0)
    ...
    // curLevel has all the trie nodes at the depth 4 that are under
    // the trie node with this prefix.
    for _, tn := range curLevel {
        words = append(words, tn.wordsWithPrefix...)
    }
